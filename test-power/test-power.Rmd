...
This _playground_ illustrates the distribution of the outputs of a sample of statistical tests. It is intended to explore the artifacts that may arise when statistical power is low.
...
calculate <- reactive({
  ...
  muestra <- rnorm(n_test, effect_size, data_sd)
  out <- replicate(n_iter, do_one_test(muestra))
  out <- data.frame(t(out))
  colnames(out) <- c("p_value", "ci_lower", "ci_upper", "estimated")
  
  do_tests <- function() {
    #muestra <- rnorm(n_test, effect_size, data_sd)
    out <- replicate(n_iter, do_one_test(rnorm(n_test, effect_size, data_sd)))
    out <- data.frame(t(out))
    colnames(out) <- c("p_value", "ci_lower", "ci_upper", "estimate")
...
### Proportion of significant tests (aka, power) {.value-box}
...
### Distribution of estimated effects for the _significant tests_ only
